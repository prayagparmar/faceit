...
public class EigenfaceRecognition implements IRecognitionPlugin, ISwingCustomizable {
	...
	public synchronized HashMap<String, Integer> getRecognitionPoints(Region region, FaceDatabase faceDB, String imageFolderPath) {
		
		// Ensure an instance of EigenfaceBuilder is present
		if (eigenfaceBuilder == null)
			eigenfaceBuilder = new EigenfaceBuilder(this, faceDB, imageFolderPath);
		...
		String[] names = faceDB.getExistingAnnotations();
		System.out.println("Existing names" + names[0]);
		
		BufferedImage unknownFaceImage = region.toThumbnail(Constants.FACE_THUMBNAIL_SIZE, imageFolderPath);
		...
		for (String name : names) {
			Region image = null;
			Region[] regionsForName = faceDB.getRegionsForFace(name);
					...
					if (regionsForName[i] != null && regionsForName[i] != region && regionsForName[i].isUsedForTraining()) {
						...
						byte[] knownFace = Utilities
								.bufferedImageToIntensityArray(regionsForName[i]
										.toThumbnail(Constants.FACE_THUMBNAIL_SIZE, imageFolderPath));
