...
public class EigenfaceBuilder {
	 ...
	 */
	public EigenfaceBuilder(EigenfaceRecognition eigenfaceRecognition, FaceDatabase faceDB, String imageFolderPath){
	...
	/**
	 * Custom method written for Faceit. This uses customized FaceDB passed as an argument.
	 * @param faceDB
	 */
	public void updateEigenfaces(){
		
		Region[] knownFaces = faceDB.getTrainingImages();
		System.out.println("\n\nTraining Images length: " + knownFaces.length + "\n");
		int numTrainingImages = knownFaces.length;
		
		int lastNumTrainingImages = eigenfaceRecognition.lastNumberOfTrainingImages;
		System.out.println("Last Number Training Images length: " + lastNumTrainingImages + "\n\n");
			...
			(step != null && Math.abs(numTrainingImages - lastNumTrainingImages) > step))
		{
			if (knownFaces.length > 0) {
				...
				for(int r = 0; r < knownFaces.length; r++){
					...
					byte[] intensityImage = Utilities.bufferedImageToIntensityArray(knownFaces[r].toThumbnail(Constants.FACE_THUMBNAIL_SIZE, imageFolderPath));
				...
				System.out.println("Average face: " + averageFace.length);
				System.out.println("Eigen face: " + eigenFaces.size());
				System.out.println("Training images: " + numTrainingImages);
...
//	}
	...
	/**
	 * Class written for Faceit
	 */
//	public void updateEigenfacesInBackgroundFaceIt(FaceDatabase faceDB) {
//		(new BackgroundWorkerFaceIt(faceDB)).execute();
//	}
//	
//	class BackgroundWorkerFaceIt extends SwingWorker{
//		
//		FaceDatabase faceDB;
//		
//		public BackgroundWorkerFaceIt(FaceDatabase faceDB){
//			this.faceDB = faceDB;
//		}
//		
//		/* (non-Javadoc)
